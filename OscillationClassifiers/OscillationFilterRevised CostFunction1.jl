...
where getdiff is divided by the number of peaks was used.

Compared to the original cost function without this change, the number of false positives that get
past the cost function is quartered, but the number of false negatives is increased from 1/60
to 5/60. As a result, we can conclude that the new cost function is better at eliminating damped
solutions, but it can also lead to more oscillatory solutions being incorrectly weeded out.
-----------
=#
...
"""
function newgetSTD(peakindxs::Vector{Int}, arrayData::Vector{Float64})
...
function new_getDif(indexes::Vector{Int}, arrayData::Vector{Float64}) #get difference between fft peak indexes
    arrLen = length(indexes)
    if arrLen < 2
        return 0.0
    end
...
function CostFunction(Y::ODESolution)
    ...
    diff = new_getDif(fftindexes, fftData) #get the difference between the peaks
    return std + diff
end
...
for i in 1:numIterations
    ...
    write("/Users/ezragreenberg/NewCostFunctionFilter/SeedConcentrations.txt","$([i for i in u0[2:5]])\n","a")
    if eval_fitness_catcherrors(currentSol) < -0.01
        if simpleOscillationCheck(currentSol, 80.)
           savefig("/Users/ezragreenberg/NewCostFunctionFilter/Pass/$name")
        else
            savefig("/Users/ezragreenberg/NewCostFunctionFilter/Fail80/$name")
        end
        ...
        savefig("/Users/ezragreenberg/NewCostFunctionFilter/FailCost/$name")
    end
    if i % 500 == 0 
