...
deviation fo the data points varied by more than 0.1% of the mean of the data points. The cost function was changed from the original 
by getting rid of the 1/P in getSTD. The summation in getDIF was also simplified to just take the height of the first peak.

When this was done with diff commented out of the cost function, there were 7 false negatives, no false positives, 
and nothing was in the Fail80 folder.

With diff, 4 false negatives, 256 in fail 80. Note that when the window size used for fftindexes in the
cost function is changed to 10 in the very first thing checked, the number of false
negatives increases from 1 to 5.
...
and the data points one index above and below. The standard deviations are then summed. This is equivalent 
to the first term of the cost function in equation 4 of Pušnik et al, but without 1/P. Very 
...
"""
function noAvGetSTD(peakindxs::Vector{Int}, arrayData::Vector{Float64})
...
function CostFunction(Y::ODESolution)
    ...
    std = noAvGetSTD(fftindexes, fftData) #get the standard deviation of the peaks
...
for i in 1:numIterations
    ...
    if eval_fitness_catcherrors(currentSol) < -0.01
        if simpleOscillationCheck(currentSol, 80.)
           #savefig("~/NoAverageCost/Pass/$name")
           successCount += 1
        else
            #savefig("~/NoAverageCost/Fail80/$name")
        end
        
    else
        #savefig("~/NoAverageCost/FailCost/$name")
    end
    if i % 500 == 0 
