...
#         :kb7 => 39. * ka7 - kcat7, :kcat7 => 85.3, :df => 0.]
# p = [x[2] for x in psym]
...
function constructParams(theka1, thekb1, theka4, theka7, thedf)
        thepsym = [:ka1 => theka1, :kb1 => thekb1, :kcat1 => 15. * theka1 - thekb1, :ka2 => 0.7*10^-3, :kb2 => 2.0*10^-3,
                 ...
                 :kb7 => 39. * ka7 - kcat7, :kcat7 => 85.3, :df => thedf]
...
for i in 1:size(record)[1]
        ...
        plot(sol[1,:])
        savefig("graphStorage/Graph$(round(stdVal / meanVal, sigdigits = 3, base = 10)),$(record.fit[i]).png")
end
